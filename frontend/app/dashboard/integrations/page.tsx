...
function ConnectPaystackModal({ onClose, onSuccess, brandId }: ConnectModalProps) {
    ...
    const handleConnect = async (e: React.FormEvent) => {
        ...
        setError("")
        try {
            await api.post("/api/integrations/paystack/connect", {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
            <div className="w-full max-w-md rounded-2xl p-6 space-y-5" style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-base font-bold font-display" style={{ color: "var(--color-text-primary)" }}>Connect Paystack</h2>
...
export default function IntegrationsPage() {
    ...
    useEffect(() => { loadIntegrations() }, [activeBrandId])

    const paystackIntegration = integrations.find(i => i.type === "paystack")
    ...
    return (
        <div className="p-8 max-w-3xl space-y-8">
            ...
            <div className="rounded-2xl p-6" style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
                <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        {/* Paystack brandmark placeholder */}
                        <div className="w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold" style={{ backgroundColor: "rgba(0,201,167,0.1)", color: "var(--color-accent)", fontFamily: "var(--font-display)" }}>
                            P
                        </div>
                        <div>
                            <h3 className="font-bold font-display" style={{ color: "var(--color-text-primary)" }}>Paystack</h3>
                            <p className="text-sm font-body" style={{ color: "var(--color-text-muted)" }}>Payment gateway — Nigeria, Ghana, South Africa</p>
                        </div>
                    </div>

                    {/* Status badge */}
                    {paystackIntegration?.status === "connected" ? (
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-4">
                        <div className="rounded-xl p-4" style={{ backgroundColor: "var(--color-surface-raised)" }}>
                            ...
                            <p className="mt-1 text-sm font-semibold font-body" style={{ color: "var(--color-text-primary)" }}>
                                {formatDate(paystackIntegration.last_sync_at)}
                            </p>
                        </div>
                        <div className="rounded-xl p-4" style={{ backgroundColor: "var(--color-surface-raised)" }}>
                            <p className="text-xs font-body" style={{ color: "var(--color-text-muted)" }}>Orders imported</p>
                            <p className="mt-1 text-sm font-semibold font-body" style={{ color: "var(--color-text-primary)" }}>
                                {paystackIntegration.transaction_count.toLocaleString()}
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="mt-5 flex gap-3">
                    {paystackIntegration?.status === "connected" ? (
                        ...
                        <button onClick={() => setShowModal(true)}
                            ...
                            style={{ backgroundColor: "var(--color-accent)", color: "#0A0A0F" }}>
                            <Plus className="w-4 h-4" /> Connect Paystack
                        </button>
                    )}
                </div>
            </div>
            ...
            {["Flutterwave", "Monnify", "OPay"].map(name => (
                <div key={name} className="rounded-2xl p-6 opacity-50" style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold" style={{ backgroundColor: "var(--color-surface-raised)", color: "var(--color-text-muted)", fontFamily: "var(--font-display)" }}>
                                {name[0]}
                            </div>
                            <div>
                                <h3 className="font-bold font-display" style={{ color: "var(--color-text-primary)" }}>{name}</h3>
                                <p className="text-sm font-body" style={{ color: "var(--color-text-muted)" }}>Coming soon</p>
                            </div>
                        </div>
                        <span className="px-3 py-1 rounded-full text-xs font-semibold font-body"
                            style={{ backgroundColor: "var(--color-surface-raised)", color: "var(--color-text-muted)", border: "1px solid var(--color-border)" }}>
